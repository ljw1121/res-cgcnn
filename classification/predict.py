...
def validate(val_loader, model, criterion, normalizer, test=False):
    ...
    for i, (input, target, batch_cif_ids) in enumerate(val_loader):
       ...
       # output = torch.where(output<=0,torch.tensor(0,dtype=torch.float),output)
    ...
    if test:
        star_label = '**'
        import csv
        with open('pre_fermi_test_results_0.86!!!.csv', 'w') as f:
            writer = csv.writer(f)
...
def save_checkpoint(state, is_best, filename='pre_checkpoint.pth.tar'):
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, 'pre_model_best.pth.tar')
