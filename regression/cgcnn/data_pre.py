...
def get_train_val_test_loader(dataset, collate_fn=default_collate,
    ...
    train_sampler = SubsetRandomSampler(indices[:train_size])   # 采样样本元素
...
class GaussianDistance(object):
    ...
    def __init__(self, dmin, dmax, step, var=None):
        ...
        """
        assert dmin < dmax         # assert的作用是现计算表达式 expression ，如果其值为假（即为0），那么它先向stderr打印一条出错信息，然后通过调用 abort 来终止程序运行
        assert dmax - dmin > step  # assert宏的原型定义在<assert.h>中，其作用是如果它的条件返回错误，则终止程序执行
...
class AtomCustomJSONInitializer(AtomInitializer):  # 读取feature文件
    """
...
class CIFData(Dataset):  # 读取cif文件
    ...
    def __init__(self, root_dir, max_num_nbr=12, radius=8, dmin=0, step=0.2,
        ...
        id_prop_file = os.path.join(self.root_dir, 'id_prop.csv')   # csv文件地址
        assert os.path.exists(id_prop_file), 'id_prop.csv does not exist!'
        with open(id_prop_file) as f:        # 读取csv文件
            reader = csv.reader(f)
            self.id_prop_data = [row for row in reader]
        random.seed(random_seed)
        random.shuffle(self.id_prop_data)
        atom_init_file = os.path.join(self.root_dir, 'atom_init.json')    # json文件地址
    ...
    @functools.lru_cache(maxsize=None)  # Cache loaded structures缓存加载的结构
    def __getitem__(self, idx):
        cif_id = self.id_prop_data[idx]
    
        crystal = Structure.from_file(os.path.join(self.root_dir,
                                                   str(cif_id[0])+'.cif'))      # 读取cif文件，晶体结构

        atom_fea = np.vstack([self.ari.get_atom_fea(crystal[i].specie.number)  # vstack追加数组，这里读了晶体结构，应该是原子信息，可以看一下
                              for i in range(len(crystal))])
        atom_fea = torch.Tensor(atom_fea)
        all_nbrs = crystal.get_all_neighbors(self.radius, include_index=True)  # 这里也读了晶体结构，应该是边信息
